...
RabbitMQ.prototype.subscribe = function(callback, needAck){
    var def = Q.defer();
    var _then = this;
...
RabbitMQ.prototype.assertQueue = function(){
    ...
    if(this.exchanger && this.exchanger !== ''){
        return this.getConnect().then(function(ch){
           channel = ch;
           return ch.assertExchange(_then.exchanger, _then.exchanger_type || "fanout");
        })
        .then(function(){
        ...
        return _then.getConnect().then(function(ch){
            return ch.assertQueue(_then.queue, _then.queue_options);
        });
    }
